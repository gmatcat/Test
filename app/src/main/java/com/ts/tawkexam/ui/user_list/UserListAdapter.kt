...
class UserListAdapter(private val users: ArrayList<User>, private val listener: UserListCallback) :
    RecyclerView.Adapter<UserListAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        ViewHolder(parent.inflater(R.layout.user_item))
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(
            ...
            val inverted = position > 0 && ((position + 1) % 4 == 0)
            if(inverted) {
            ...
            itemView.onClick { listener.onClickUser(user, inverted) }
